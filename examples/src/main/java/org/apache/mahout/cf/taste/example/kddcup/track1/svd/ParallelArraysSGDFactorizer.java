...
/**
 * {@link Factorizer} based on Simon Funk's famous article <a href="http://sifter.org/~simon/journal/20061211.html"> "Netflix Update: Try this at home"</a>.
 *
 * Attempts to be as memory efficient as possible, only iterating once through the {@link FactorizablePreferences} or {@link DataModel} while copying everything to primitive arrays. Learning works in place on these
...
public class ParallelArraysSGDFactorizer implements Factorizer {
	...
	public ParallelArraysSGDFactorizer(DataModel dataModel, int numFeatures, int numIterations) {
		this(new DataModelFactorizablePreferences(dataModel), numFeatures, numIterations, DEFAULT_LEARNING_RATE, DEFAULT_PREVENT_OVERFITTING, DEFAULT_RANDOM_NOISE);
	}
	...
	public ParallelArraysSGDFactorizer(FactorizablePreferences factorizablePrefs, int numFeatures, int numIterations) {
		this(factorizablePrefs, numFeatures, numIterations, DEFAULT_LEARNING_RATE, DEFAULT_PREVENT_OVERFITTING, DEFAULT_RANDOM_NOISE);
	}
		...
		for (int feature = 0; feature < numFeatures; feature++) {
			...
			if (feature < numFeatures - 1) {
				...
				for (int index = 0; index < userIndexes.length; index++) {
					cachedEstimates[index] = estimate(userIndexes[index], itemIndexes[index], feature, cachedEstimates[index], false);
				}
			}
		}
