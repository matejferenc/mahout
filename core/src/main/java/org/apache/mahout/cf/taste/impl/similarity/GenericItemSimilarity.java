...
/**
 * <p>
 * A "generic" {@link ItemSimilarity} which takes a static list of precomputed item similarities and bases its responses on that alone. The values may have been precomputed offline by another process, stored in a file,
 * and then read and fed into an instance of this class.
 * </p>
 * 
 * <p>
 * This is perhaps the best {@link ItemSimilarity} to use with {@link org.apache.mahout.cf.taste.impl.recommender.GenericItemBasedRecommender}, for now, since the point of item-based recommenders is that they can take
 * advantage of the fact that item similarity is relatively static, can be precomputed, and then used in computation to gain a significant performance advantage.
 * </p>
 */
public final class GenericItemSimilarity implements ItemSimilarity {
	...
	/**
	 * <p>
	 * Creates a {@link GenericItemSimilarity} from a precomputed list of {@link ItemItemSimilarity}s. Each represents the similarity between two distinct items. Since similarity is assumed to be symmetric, it is not
	 * necessary to specify similarity between item1 and item2, and item2 and item1. Both are the same. It is also not necessary to specify a similarity between any item and itself; these are assumed to be 1.0.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * <p>
	 * Like {@link #GenericItemSimilarity(Iterable)}, but will only keep the specified number of similarities from the given {@link Iterable} of similarities. It will keep those with the highest similarity -- those that
	...
	public GenericItemSimilarity(Iterable<ItemItemSimilarity> similarities, int maxToKeep) {
		Iterable<ItemItemSimilarity> keptSimilarities = TopItems.getTopItemItemSimilarities(maxToKeep, similarities.iterator());
	 ...
	 * 
	 * <p>
	 * It's valid to build a {@link GenericItemSimilarity} this way, but perhaps missing some of the point of an item-based recommender. Item-based recommenders use the assumption that item-item similarities are
	 * relatively fixed, and might be known already independent of user preferences. Hence it is useful to inject that information, using {@link #GenericItemSimilarity(Iterable)}.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Like {@link #GenericItemSimilarity(ItemSimilarity, DataModel)} )}, but will only keep the specified number of similarities from the given {@link DataModel}. It will keep those with the highest similarity -- those
	...
	/**
	 * <p>
	 * Returns the similarity between two items. Note that similarity is assumed to be symmetric, that {@code itemSimilarity(item1, item2) == itemSimilarity(item2, item1)}, and that
