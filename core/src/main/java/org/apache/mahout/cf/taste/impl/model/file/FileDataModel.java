...
/**
 * <p>
 * A {@link DataModel} backed by a delimited file. This class expects a file where each line contains a user ID, followed by item ID, followed by optional preference value, followed by optional timestamp. Commas or tabs
 * delimit fields:
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * The preference value is assumed to be parseable as a {@code double}. The user IDs and item IDs are read parsed as {@code long}s. The timestamp, if present, is assumed to be parseable as a {@code long}, though this can
 * be overridden via {@link #readTimestampFromString(String)}. The preference value may be empty, to indicate "no preference value", but cannot be empty. That is, this is legal:
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * This class will also look for update "delta" files in the same directory, with file names that start the same way (up to the first period). These files have the same format, and provide updated data that supersedes
 * what is in the main data file. This is a mechanism that allows an application to push updates to {@link FileDataModel} without re-copying the entire data file.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * Note that it's all-or-nothing -- all of the items in the file must express no preference, or the all must. These cannot be mixed. Put another way there will always be the same number of delimiters on every line of the
 * file!
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * It is possible and likely useful to subclass this class and customize its behavior to accommodate application-specific needs and input formats. See {@link #processLine(String, FastByIDMap, FastByIDMap, boolean)} and
...
public class FileDataModel extends AbstractDataModel {
	...
	/**
	 * @param dataFile
	 *            file containing preferences data. If file is compressed (and name ends in .gz or .zip accordingly) it will be decompressed as it is read)
	 ...
	 *            If your data file don't use '\t' or ',' as delimiter, you can specify a custom regex pattern.
	 */
	...
	protected DataModel buildModel() throws IOException {
		...
		if (hasPrefValues) {
			...
			throw new IllegalStateException("no pref values in dataset. is dataset boolean?");
		}
	}

	/**
	 * Finds update delta files in the same directory as the data file. This finds any file whose name starts the same way as the data file (up to first period) but isn't the data file itself. For example, if the data
	...
	private Iterable<File> findUpdateFilesAfter(long minimumLastModified) {
		...
		for (File updateFile : parentDir.listFiles(onlyFiles)) {
			...
			if (updateFileName.startsWith(startName) && !updateFileName.equals(dataFileName) && updateFile.lastModified() >= minimumLastModified) {
	...
	protected void processLine(String line, FastByIDMap<?> data, FastByIDMap<FastByIDMap<Long>> timestamps, boolean fromPriorData) {
		...
		int userID = readUserIDFromString(userIDString);
		int itemID = readItemIDFromString(itemIDString);
	...
	private void addTimestamp(int userID, int itemID, String timestampString, FastByIDMap<FastByIDMap<Long>> timestamps) {
	...
	/**
	 * Subclasses may wish to override this if ID values in the file are not numeric. This provides a hook by which subclasses can inject an {@link org.apache.mahout.cf.taste.model.IDMigrator} to perform translation.
	 */
	protected int readUserIDFromString(String value) {
	 ...
	 * Subclasses may wish to override this if ID values in the file are not numeric. This provides a hook by which subclasses can inject an {@link org.apache.mahout.cf.taste.model.IDMigrator} to perform translation.
	 */
	protected int readItemIDFromString(String value) {
		return Integer.parseInt(value);
	}
	 ...
	 */
	protected int readTimestampFromString(String value) {
		return Integer.parseInt(value);
	}
	...
	/**
	 * Note that this method only updates the in-memory preference data that this {@link FileDataModel} maintains; it does not modify any data on disk. Therefore any updates from this method are only temporary, and lost
	...
	public void refresh(Collection<Refreshable> alreadyRefreshed) {
		if (dataFile.lastModified() > lastModified + minReloadIntervalMS || readLastUpdateFileModified() > lastUpdateFileModified + minReloadIntervalMS) {
