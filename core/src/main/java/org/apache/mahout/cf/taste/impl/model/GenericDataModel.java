...
public final class GenericDataModel extends AbstractDataModel {
	 ...
	 * 
	 * @param userData
	 *            users to include; (see also {@link #toDataMap(FastByIDMap, boolean)})
	 */
	 ...
	 *            users to include; (see also {@link #toDataMap(FastByIDMap, boolean)})
	 * @param timestamps
	 *            optionally, provided timestamps of preferences as milliseconds since the epoch. User IDs are mapped to maps of item IDs to Long timestamps.
	 */
	public GenericDataModel(FastByIDMap<PreferenceArray> userData, FastByIDMap<FastByIDMap<Long>> timestamps) {
		...
		for (Map.Entry<Integer, PreferenceArray> entry : preferenceFromUsers.entrySet()) {
			PreferenceArray prefs = entry.getValue();
			prefs.sortByItem();
			for (Preference preference : prefs) {
				int itemID = preference.getItemID();
		...
		for (Map.Entry<Integer, PreferenceArray> entry : preferenceForItems.entrySet()) {
	 ...
	 * @deprecated without direct replacement. Consider {@link #toDataMap(DataModel)} with {@link #GenericDataModel(FastByIDMap)}
	 */
	@Deprecated
	...
	public static FastByIDMap<PreferenceArray> toDataMap(FastByIDMap<Collection<Preference>> data, boolean byUser) {
		for (Map.Entry<Integer, Object> entry : ((FastByIDMap<Object>) (FastByIDMap<?>) data).entrySet()) {
	...
	@Override
	public IntPrimitiveArrayIterator getUserIDs() {
		return new IntPrimitiveArrayIterator(userIDs);
	}
	...
	@Override
	public IntPrimitiveArrayIterator getItemIDs() {
		return new IntPrimitiveArrayIterator(itemIDs);
	}
