...
public final class OrderBasedRecommenderEvaluator {
	...
	public static void evaluate(Recommender recommender1, Recommender recommender2, int samples, RunningAverage tracker, String tag) throws TasteException {
		printHeader();
		IntPrimitiveIterator users = recommender1.getDataModel().getUserIDs();
	...
	public static void evaluate(Recommender recommender, DataModel model, int samples, RunningAverage tracker, String tag) throws TasteException {
		printHeader();
		IntPrimitiveIterator users = recommender.getDataModel().getUserIDs();
		while (users.hasNext()) {
			long userID = users.nextInt();
	...
	public static void evaluate(DataModel model1, DataModel model2, int samples, RunningAverage tracker, String tag) throws TasteException {
		printHeader();
		IntPrimitiveIterator users = model1.getUserIDs();
		while (users.hasNext()) {
			long userID = users.nextInt();
	...
	/**
	 * This exists because FastIDSet has 'retainAll' as MASK, but there is no count of the number of items in the set. size() is supposed to do this but does not work.
	 */
	...
	private static Integer[] getCommonItems(FastIDSet commonSet, Iterable<RecommendedItem> recs, int max) {
		Integer[] commonItems = new Integer[max];
		int index = 0;
		for (RecommendedItem rec : recs) {
			Integer item = rec.getItemID();
	...
	private static Integer[] getCommonItems(FastIDSet commonSet, PreferenceArray prefs1, int max) {
		Integer[] commonItems = new Integer[max];
		int index = 0;
		for (int i = 0; i < prefs1.length(); i++) {
			Integer item = prefs1.getItemID(i);
			if (commonSet.contains(item)) {
				commonItems[index++] = item;
			}
			if (index == max) {
	 ...
	 *
	 * These measurements are given the set of results that are common to both recommendation lists. They only get ordered lists.
	 *
	 * These measures all return raw numbers do not correlate among the tests. The numbers are not corrected against the total number of samples or the number of common items. The one contract is that all measures are 0
	 * for an exact match and an increasing positive number as differences increase.
	 */
	private static double scoreCommonSubset(String tag, long userID, int samples, int subset, Long[] itemsL, Long[] itemsR) {
		...
		log.info("{},{},{},{},{},{},{},{},{}", tag, userID, samples, subset, hamming, bubble, meanRank, normalW, variance);
		return variance;
	}
	...
	/**
	 * Normal-distribution probability value for matched sets of values. Based upon: http://comp9.psych.cornell.edu/Darlington/normscor.htm
	 * 
	...
	/**
	 * vector Z is a list of distances between the correct value and the recommended value Z[i] = position i of correct itemID - position of correct itemID in recommendation list can be positive or negative the smaller
	 * the better - means recommendations are closer both are the same length, and both sample from the same set
	 * 
	...
	/**
	 * Ranks are the position of the value from low to high, divided by the # of values. I had to walk through it a few times.
	 */
	...
	/**
	 * Do bubble sort and return number of swaps needed to match preference lists. Sort itemsR using itemsL as the reference order.
	 */
