 ...
 *
 * <p>
 * This implementation maintains two parallel arrays, of item IDs and values. The idea is to save allocating {@link Preference} objects themselves. This saves the overhead of {@link Preference} objects but also
...
public final class GenericUserPreferenceArray implements PreferenceArray {
	...
	public GenericUserPreferenceArray(int size) {
		...
		this.id = Integer.MIN_VALUE; // as a sort of 'unspecified' value
	}
	 ...
	 */
	private GenericUserPreferenceArray(Integer[] ids, int id, Float[] values) {
		this.ids = ids;
		this.id = id;
		this.values = values;
	}
	 ...
	 */
	@Override
	public void setUserID(int i, Integer userID) {
		id = userID;
	}
	...
	@Override
	public void setItemID(int i, Integer itemID) {
		ids[i] = itemID;
	}
	...
	@Override
	public boolean hasPrefWithUserID(Integer userID) {
		return id.equals(userID);
	}

	@Override
	public boolean hasPrefWithItemID(Integer itemID) {
		for (int id : ids) {
			if (itemID == id) {
				return true;
			}
