...
public interface DataModel extends Refreshable, Serializable {
	...
	/**
	 * @return a {@link IntPrimitiveIterator} of all item IDs in the model, in order
	 * @throws TasteException
	 *             if an error occurs while accessing the data
	 */
	 ...
	 *             if an error occurs while accessing the data
	 */
	double getPreferenceValue(Integer userID, Integer itemID) throws TasteException;
	 ...
	 *
	 * @param userID
	 *            user ID for preference in question
	 * @param itemID
	...
	/**
	 * @return total number of items known to the model. This is generally the union of all items preferred by at least one user but could include more.
	 * @throws TasteException
	 *             if an error occurs while accessing the data
	 */
	...
	/**
	 * @return true if this implementation actually stores and returns distinct preference values; that is, if it is not a 'boolean' DataModel
	 */
	boolean hasPreferenceValues();

	/**
	 * @return the maximum preference value that is possible in the current problem domain being evaluated. For example, if the domain is movie ratings on a scale of 1 to 5, this should be 5. While a
	 *         {@link org.apache.mahout.cf.taste.recommender.Recommender} may estimate a preference value above 5.0, it isn't "fair" to consider that the system is actually suggesting an impossible rating of, say, 5.4
	 *         stars. In practice the application would cap this estimate to 5.0. Since evaluators evaluate the difference between estimated and actual value, this at least prevents this effect from unfairly penalizing a
